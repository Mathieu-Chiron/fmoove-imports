...
Aucun appel réseau réel : _post est remplacé par un double qui capture ou
...
def test_poste_le_csv_en_base64_sans_saut_de_ligne(monkeypatch):
    ...
    monkeypatch.setattr(payt_api, "_post", fake_post)
...
def test_leve_une_erreur_si_reponse_non_2xx(monkeypatch):
    monkeypatch.setattr(
        payt_api, "_post", lambda url, **kw: FakeResponse(422, "champ manquant")
    )
...
def test_leve_une_erreur_si_reseau_indisponible(monkeypatch):
    ...
    monkeypatch.setattr(payt_api, "_post", boom)
...
def test_rejoue_sur_429_puis_reussit(monkeypatch):
    ...
    monkeypatch.setattr(payt_api, "_post", post)
...
def test_transmet_le_proxy_a_l_appel(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        payt_api, "_post", lambda url, **kw: captured.update(kw) or FakeResponse(200)
    )

    upload_csv("x", "f.csv", **_args(proxy="http://fixie:pw@proxy.usefixie.com:80"))

    assert captured["proxy"] == "http://fixie:pw@proxy.usefixie.com:80"


def test_pas_de_proxy_par_defaut(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        payt_api, "_post", lambda url, **kw: captured.update(kw) or FakeResponse(200)
    )
